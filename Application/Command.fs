...
module internal Command =
    ...
    type SaveDeviceGroupKey = { Key: Security.DeviceGroupKey }

    type SaveSensorKey = { Key: Security.SensorKey }
    ...
    let private saveDeviceGroupKeyEvent (command: SaveDeviceGroupKey) =
        let event: Event.SavedDeviceGroupKey = { Key = command.Key }
        Event.SavedDeviceGroupKey event

    let private saveSensorKeyEvent (command: SaveSensorKey) =
        let event: Event.SavedSensorKey = { Key = command.Key }
    ...
    let private createEventFromCommand (command: Command) =
        task {
            ...
            | ChangeSensorState changeSensorState -> return! sensorStateChangedEvent changeSensorState

            | ChangeSensorName changeSensorName -> return sensorNameChangedEvent changeSensorName

            | SaveDeviceGroupKey saveDeviceGroupKey -> return saveDeviceGroupKeyEvent saveDeviceGroupKey

            | SaveSensorKey saveSensorKey -> return saveSensorKeyEvent saveSensorKey
        }
