...
module SensorStateStorage =
    ...
    let private GetSensorExpression (deviceGroupId: string) (sensorId: string) =
        ...
        let expr =
            Expressions.Lambda.Create<StorableSensorState> (fun x ->
    ...
    let private GetSensorsExpression (deviceGroupId: string) =
        ...
        let expr =
            Expressions.Lambda.Create<StorableSensorState>(fun x -> x.DeviceGroupId = deviceGroupId)
    ...
    let StoreSensorName (deviceGroupId: string) (sensorId: string) (sensorName: string) =
        ...
        async {
            do!
                SensorsCollection.UpdateOneAsync<StorableSensorState>(filter, update)
                |> Async.AwaitTask
                |> Async.Ignore
        }
    ...
    let StoreSensorState (sensorState: StorableSensorState) =
        ...
        let update =
            Builders<StorableSensorState>
                .Update.Set((fun s -> s.DeviceGroupId), sensorState.DeviceGroupId)
    ...
    let GetSensorState deviceGroupId sensorId : Async<StorableSensorState> =
        async {
            ...
            let! sensorState =
                SensorsCollection
                    .FindSync<StorableSensorState>(filter)
