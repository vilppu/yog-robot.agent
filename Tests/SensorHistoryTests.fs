...
module SensorHistoryTests =
    ...
    let AuthenticationTokenIsChecked () =
        ...
        let response =
            context
            |> GetExampleSensorHistoryResponse "device-1.rh"
    ...
    let SensorHistoryContainsMeasurements () =
        ...
        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice example deviceId)
    ...
    let SensorHistoryContainsMeasurementsChronologically () =
        ...
        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice (Measurement.RelativeHumidity 78.0) deviceId)

        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice (Measurement.RelativeHumidity 80.0) deviceId)

        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice (Measurement.RelativeHumidity 79.0) deviceId)
    ...
    let SensorHistoryEntryContainsTimestamp () =
        ...
        Assert.True(
            (System.DateTime.UtcNow - entry.Timestamp)
                .TotalMinutes < 1.0
        )
    ...
    let DeviceSpecificMeasurementHistory () =
        ...
        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice anotherExample deviceId)
    ...
    let MeasurementHistoryShouldBeLimitedToContainOnlyNearHistory () =
        ...
        for i in 1..bigNumberOfEvents do
            ...
            context
            |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice measurement deviceId)

        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice latest deviceId)
    ...
    let StoreOnlyMeasurementChanges () =
        ...
        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice (Measurement.RelativeHumidity 77.0) deviceId)
        ...
        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice (Measurement.RelativeHumidity 81.0) deviceId)

        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice (Measurement.RelativeHumidity 82.0) deviceId)
    ...
    let ShowHistoryPerDevice () =
        ...
        context
        |> WriteMeasurementSynchronously(Fake.MeasurementFromDevice anotherExample "device-2")
